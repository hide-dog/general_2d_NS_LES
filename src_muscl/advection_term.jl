...
# ------------------------------------
function AUSM(E_adv_hat, F_adv_hat, QbaseU, QbaseD, QbaseL, QbaseR, QconU, QconD, QconL, QconR, cellxmax, cellymax, vecAx, vecAy, 
    ...
    for j in 1+icell:cellymax -icell
        for i in 1+icell:cellxmax+1 -icell
            ...
            rhoL = QbaseL[i,j,1]
            UL = QbaseL[i,j,2]*vAxx + QbaseL[i,j,3]*vAxy
            pL = QbaseL[i,j,4]
            ...
            rhoR = QbaseR[i,j,1]
            UR = QbaseR[i,j,2]*vAxx + QbaseR[i,j,3]*vAxy
            pR = QbaseR[i,j,4]
            ...
            elseif ad_scheme == 2
                mdot, ph = AUSM_plusup_half(rhoL, rhoR, UL, UR, pL, pR, Minf, g, i, j)
            elseif ad_scheme == 4
                velocity = (0.5 * (QbaseL[i,j,2]^2 + QbaseL[i,j,3]^2 + QbaseR[i,j,2]^2 + QbaseR[i,j,3]^2))^0.5
            ...
            for l in 1:nval
                Lpsi[l] = QconL[i,j,l] / QconL[i,j,1]
                Rpsi[l] = QconR[i,j,l] / QconR[i,j,1]
            end
            Lpsi[4] = (QconL[i,j,4] + QbaseL[i,j,4]) / QconL[i,j,1]
            Rpsi[4] = (QconR[i,j,4] + QbaseR[i,j,4]) / QconR[i,j,1]
    ...
    for j in 1+icell:cellymax+1 -icell
        for i in 1+icell:cellxmax -icell
            ...
            rhoD = QbaseD[i,j,1]
            VD = QbaseD[i,j,2]*vAyx + QbaseD[i,j,3]*vAyy
            pD = QbaseD[i,j,4]
            ...
            rhoU = QbaseU[i,j,1]
            VU = QbaseU[i,j,2]*vAyx + QbaseU[i,j,3]*vAyy
            pU = QbaseU[i,j,4]
            ...
            for l in 1:nval
                Lpsi[l] = QconD[i,j,l] / QconD[i,j,1]
                Rpsi[l] = QconU[i,j,l] / QconU[i,j,1]
            end
            Lpsi[4] = (QconD[i,j,4] + QbaseD[i,j,4]) / QconD[i,j,1]
            Rpsi[4] = (QconU[i,j,4] + QbaseU[i,j,4]) / QconU[i,j,1]
