...
function set_initQbase(Qbase, cellxmax, cellymax, restart_file, init_rho, init_u, init_v, init_p, init_T,
    ...
    if restart_check == 1
        ...
        restartnum = 0
        output_result(0, Qbase, cellxmax, cellymax, specific_heat_ratio, out_file_front, out_ext, out_dir, Rd, nval, icell)
    end
...
# ------------------------------------
function check_divrege(Qbase, cellxmax, cellymax, Rd, fwrite, icell)
    ite = 0
    for j in 1+icell:cellymax-icell
        for i in 1+icell:cellxmax-icell        
